...
int nprocs, nthreads, dflag, flags[MAX_SIZE], option, next, play;
clock_t gstart;
...
void threadSuspendAll() { // Funcao que suspende o processamento de uma thread
...
void threadResumeAll() { // Funcao que resume o processamento de uma thread
	if (pthread_mutex_lock(&mutex) != 0) {
		perror("pthread_mutex_lock()");
		exit(EXIT_FAILURE);
	}
...
void *realTimeOperation(void *tid) { // Funcao que realiza uma operacao que consuma tempo real da CPU
	...
	clock_t start, end;
	time = procs[id].dtime;
	if(nthreads == nprocs) threadSuspendAll();
	lock(); nthreads++; unlock();
	threadStatus(id);
	printf("Rodando %d %lf\n", id, time);
	start = clock();
	while (1) {
...
void firstCome(int n, pthread_t threads[]) {
	...
	qsort(procs, n, sizeof(Process), comp_dt);
	next = 0;
	for (i = 0; i < n; ) {	
		end = clock();
